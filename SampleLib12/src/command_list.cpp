...
namespace sl12
{
	...
	//----
	void CommandList::SetGraphicsRootSignatureAndDynamicResource(RootSignature* pRS, const std::vector<std::vector<u32>>& rootIndices)
	{
		auto pCmdList = GetCommandList();
	...
	void CommandList::SetComputeRootSignatureAndDynamicResource(RootSignature* pRS, const std::vector<u32>& rootIndices)
	{
		...
		if (!rootIndices.empty())
			pCmdList->SetComputeRoot32BitConstants(0, (UINT)rootIndices.size(), rootIndices.data(), 0);
	}
