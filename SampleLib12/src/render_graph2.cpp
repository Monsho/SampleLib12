...
namespace sl12
{
	...
	bool RenderGraph2::Initialize(Device* pDev)
	{
		pDevice_ = pDev;
	...
	void RenderGraph2::ClearPasses()
	{
		renderPasses_.clear();
		graphEdges_.clear();
	}

	void RenderGraph2::AddPass(IRenderPass* pPass, IRenderPass* pParent)
	{
	...
	void RenderGraph2::AddPass(IRenderPass* pPass, const std::vector<IRenderPass*>& parents)
	{
#if _DEBUG
	...
	void RenderGraph2::AddExternalTexture(TransientResourceID id, Texture* pTexture, TransientState::Value state)
	{
		resManager_->AddExternalTexture(id, pTexture, state);
	}
	
	void RenderGraph2::AddExternalBuffer(TransientResourceID id, Buffer* pBuffer, TransientState::Value state)
	{
		resManager_->AddExternalBuffer(id, pBuffer, state);
	}

	void RenderGraph2::PreCompile()
	{
	...
	void RenderGraph2::CompileReuseResources(const CrossQueueDepsType& CrossQueueDeps, std::vector<TransientResourceDesc>& OutDescs, std::map<TransientResourceID, u16>& OutIDMap)
	{
	...
	void RenderGraph2::CreateCommands(const CrossQueueDepsType& CrossQueueDeps)
	{
		...
		std::vector<u16> nodeIDsWithooutParentGraphics;
		for (auto id : sortedNodeIDs_)
		{
			...
			{
				nodeIDsWithooutParentGraphics.push_back(id);
				break;
			}
		}
		if (!nodeIDsWithooutParentGraphics.empty())
		{
			Command barrierCmd;
			...
			transition.relativeNodeIDs = nodeIDsWithooutParentGraphics;
